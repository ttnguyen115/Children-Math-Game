...
let addResult, action, timeRemaining;
var playing = false;
...
let randomQuestion = () =>
{
    ...
    addResult = parseInt(firstNum + secondNum);
    let questionOutput = `${firstNum} + ${secondNum}`;
    div.innerHTML = questionOutput;
    
    let correctAnswerPosition = 1 + Math.floor(10 * Math.random());
    document.getElementById("container" + correctAnswerPosition).innerHTML = addResult; // fill one box with one correct answer

    // fill other boxes with different wrong answer
    var answer = [addResult];
    for (let i = 1; i < 11; i++)
    {
        if (i != correctAnswerPosition)
        {
            ...
            do
            {
                wrongAnswer = (1 + Math.floor(25 * Math.random())) + (1 + Math.floor(25 * Math.random()));
            }
...
function displayResult()
{
    ...
    document.getElementById("result-board-container").innerHTML = "<p>Game Over!</p><p>Your Name: Trung</p><p>Your Age: 6</p><p>Your score: </p>" + score + "<p>Your hit:</p>" + hit + "<p>Your Miss: </p>" + miss;
}
